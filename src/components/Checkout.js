          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box display="flex" flexDirection="column">
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
               ...
               {addresses.all.length>0 && addresses.all.map((address)=>(
                 <Box item key={address._id} className="address-item"
                 border={3}
                 borderColor = { addresses.selected===address._id ? '#00A278':'#ddd' }
                 backgroundColor = { addresses.selected===address._id ? "#E9F5E1":"white" }
                 mt={1}
                 >
                  <Box alignSelf="center" ml={1}>
                    <Button type="text" role="text" variant="text" sx={{color:"black"}}
                      onClick={()=>{
